...
def scrape_zeen_sales(num_pages, output_file):
...
def scrape_product_data(file_path, output_json):
    ...
    for product in products:
        ...
        if 't4s-pr-grid' in product.get("class", []) and 't4s-pr-style1' in product.get("class", []):
            ...
            Brand_name = "Zeen Clothing"
            
  
            product_data.append({
                'title': title,
                'product_page': "https://zeenwoman.com"+href,
                'image_url': image_url,
...
scrape_zeen_sales(num_pages=9, output_file='all_zeen_sales.html')

time.sleep(2)


file_path = 'all_zeen_sales.html'
output_json = 'all_zeen_sales.json'
scrape_product_data(file_path, output_json)

time.sleep(1)


os.remove("all_zeen_sales.html")
